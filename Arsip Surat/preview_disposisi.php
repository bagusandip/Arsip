...
<body onload="window.print()">
	...
	<div class="container">
		...
		<?php 
		$query = mysql_query("select * from surat_masuk where no_surat_masuk='$no_surat_masuk'") or die (mysql_error());
		$data = mysql_fetch_array($query);
		$ys = substr($data['tgl_surat'],0,4);
		$yt = substr($data['tgl_terima'],0,4);
		$ms = substr($data['tgl_surat'],5,2);
		$mt = substr($data['tgl_terima'],5,2);
		$ds = substr($data['tgl_surat'],8,2);
		$dt = substr($data['tgl_terima'],8,2);
		...
		<center>
			<table border="1px" class="backg">
				...
				<tr>
					<td class="right">Tgl. Penerimaan</td>
					<td class="left"> : <?php echo $dt." ".$nmt." ".$yt;?></td>
					<td class="right">Tgl. Penyelesaian</td>
					<td class="left"> : </td>
				</tr>
				<tr style="border : none;">
					<td  class="right bottom" >Tgl. dan No Surat</td>
					<td  colspan="3" class="left bottom">: <?php echo $ds." ".$nms." ".$ys." & ".$data['no_surat_masuk'];?></td>
				</tr>
				<tr style="border: none;">
				...
				<tr style="border: none;">
					<td class="right bottom top" >Lampiran</td>
					<td colspan="3" class="left bottom top">: <?php echo $data['lampiran'];?></td>
				</tr>
			</table>

			<table class="backg">
				<tr>
					<td style="border-top: none;" width="50%" class="tgh">Disposisi</td>
