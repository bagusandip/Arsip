...
<head>
    ...
    <script>
        ...
        function GantiQuery() {
            ...
            if (jeniscari !== "pilih" && katacari !== "" || jeniscari !== "pilih" && katacari !== null) {
                    $sql = "select * from surat_tugas where "+jenis_cari+" like '%"+katacari+"%'";
            }
        }
...
<button onclick="topFunction()" id="myBtn" title="Ke Atas"><span class="glyphicon glyphicon-arrow-up"></span></button>
	<div class="container">
	<h2>Arsip Surat Tugas<hr style="border-width: 5px; border-color: #333333"></h2>
    <ul class="breadcrumb">
        <li><a href="beranda_admin.php">Beranda</a></li>
        <li>Arsip Surat Tugas</li>
    </ul>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-3">
            <select class="form-control" id="jenis_cari" onchange="GantiField()" name="jenis_cari">
                ...
                <option value="no_surat_tugas">Nomor Surat</option>
	...
	<a href="tambah_surat_tugas.php" class="btn btn-primary btn-md">
		<span class="glyphicon glyphicon-plus-sign"></span>&nbsp Tambah Data</a>
        &nbsp 
        <a href="export_excel_stugas.php" class="btn btn-warning btn-md"> 
          <span class="glyphicon glyphicon-save-file"></span>&nbsp Export Excel</a>

	<h3>Daftar Arsip Surat Tugas</h3>
	<!-- tabel surat -->
    ...
    <thead>
      <tr style="background-color: white;">
        ...
        <th width="120px">Tanggal Keberangkatan</th>
        <th>Perihal</th>
    ...
    <tbody>
         <?php
             ...
             include 'koneksi.php';
             $sql = "select * from surat_tugas order by no_agenda";
        if (isset($_POST['filter'])) {
            ...
            if ($a !== "pilih" && $b !== "" || $a !== "pilih" && $b !== null) {
                    $sql = "select * from surat_tugas where $a like '%$b%' order by no_agenda";
            }
        }
           $tampil=mysql_query($sql);
            ...
            while($data = mysql_fetch_array($tampil)){
                 ...
                 $tanggal_keberangkatan_1= date("d-m-Y", strtotime($data['tgl_keberangkatan_1']));
                    ...
                    echo "<td>$tanggal_keberangkatan s/d $tanggal_keberangkatan_1 </td>";
                    echo "<td>{$data['perihal']}</td>";
                    echo "<td>{$data['tujuan']}</td>";
                    echo "<td>{$data['keterangan']}</td>";
                    ?>
                    <td>
                    <a href="preview_surat_tugas.php?no_surat_tugas=<?=$data['no_surat_tugas']?>" target="_blank" class="btn btn-info btn-sm" style="width: 90px; margin-bottom: 3px;">
                            <span class="glyphicon glyphicon-print"></span>&nbsp Print</a><br>
                        <a href="edit_surat_tugas.php?no_surat_tugas=<?=$data['no_surat_tugas']?>" class="btn btn-success btn-sm" style="width: 90px; margin-bottom: 3px;">
                            <span class="glyphicon glyphicon-edit"></span>&nbsp Edit</a>
                        <a onclick="return confirm('Yakin Ingin Menghapus Data ?')" href="hapus_surat_tugas.php?no_surat_tugas=<?=$data['no_surat_tugas']?>" class="btn btn-danger btn-sm" style="width: 90px; margin-bottom: 3px;">
