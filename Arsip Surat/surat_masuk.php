...
<head>
    ...
    <script>
        ...
        function GantiQuery() {
            ...
            if (jeniscari !== "pilih" && katacari !== "" || jeniscari !== "pilih" && katacari !== null) {
                    $sql = "select * from surat_masuk where "+jenis_cari+" like '%"+katacari+"%'";
            }
        }
...
<button onclick="topFunction()" id="myBtn" title="Ke Atas"><span class="glyphicon glyphicon-arrow-up"></span></button>
	<div class="container">
	<h2>Arsip Surat Masuk<hr style="border-width: 5px; border-color: #333333"></h2>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-3">
            <select class="form-control" id="jenis_cari" onchange="GantiField()" name="jenis_cari">
                ...
                <option value="no_surat_masuk">Nomor Surat</option>
...
<br><br>
<a href="tambah_surat_masuk.php" class="btn btn-primary btn-md">
        <span class="glyphicon glyphicon-plus-sign"></span>&nbsp Tambah Data</a>
<a href="export_excel_smasuk.php" class="btn btn-warning btn-md">
    ...
    <thead>
      <tr style="background-color: white;">
        ...
        <th width="110px">Tanggal Surat</th>
        <th width="130px">Tanggal Diterima</th>
        <th>Perihal</th>
    ...
    <tbody>
        ...
        <?php
             ...
             include 'koneksi.php';
             $sql = "select * from surat_masuk order by no_agenda";
        if (isset($_POST['filter'])) {
            ...
            if ($a !== "pilih" && $b !== "" || $a !== "pilih" && $b !== null) {
                    $sql = "select * from surat_masuk where $a like '%$b%' order by no_agenda";
            }
        }
           $tampil=mysql_query($sql);
            ...
            while($data = mysql_fetch_array($tampil)){
                ...
                $tanggal_terima= date("d-m-Y", strtotime($data['tgl_terima']));
                    echo "<tr class='active'>
                        <td>{$data['no_agenda']}</td>";
                    echo "<td>{$data['lampiran']}</td>";
                    echo "<td>{$data['pengirim']}</td>";
                    echo "<td>{$data['no_surat_masuk']}</td>";
                    ...
                    ?>
                    <td>
                        <a href="preview_disposisi.php?no_surat_masuk=<?=$data['no_surat_masuk']?>" target="_blank" class="btn btn-info btn-sm" style="width: 90px; margin-bottom: 3px;">
                            <span class="glyphicon glyphicon-print"></span>&nbsp Print</a><br>
                        <a href="edit_surat_masuk.php?no_surat_masuk=<?=$data['no_surat_masuk']?>" class="btn btn-success btn-sm" style="width: 90px; margin-bottom: 3px;">
                            <span class="glyphicon glyphicon-edit"></span>&nbsp Edit</a><br>
                        <a onclick="return confirm('Yakin Ingin Menghapus Data ?')" href="hapus_surat_masuk.php?no_surat_masuk=<?php echo $data['no_surat_masuk']; ?>" class="btn btn-danger btn-sm mr" style="width: 90px; margin-bottom: 3px;">
